                ...
                if (nomCentrale === "PENLY") {
                    ...
                    card.innerHTML = `
                        ...
                        <div class="penly-header">
                            ...
                            </div>const formatNum = new Intl.NumberFormat('fr-FR').format;

// Variables pour stocker les données afin de pouvoir changer le focus sans recharger la page
let donneesEnMemoire = null;
let centraleEnFocus = "PENLY"; // Focus par défaut au premier chargement
...
// Fonction qui construit toute l'interface à partir des données en mémoire
function rafraichirDashboard() {
    if (!donneesEnMemoire) return;

    const data = donneesEnMemoire;
    ...
    for (const [nomCentrale, prodTwh] of listeCentrales) {
        ...
        // Si c'est la centrale sélectionnée dans le menu déroulant
        if (nomCentrale === centraleEnFocus) {
            const { htmlUnites, nbUnites, nbEnLigne } = genererContenuCarte(nomCentrale, prodTwh, data, true);
            ...
            card.innerHTML = `
                ...
                <div class="penly-header">
                    <div class="penly-nom-row">
                        <h4 class="penly-nom"><span class="penly-badge">Focus</span> ${nomCentrale}</h4>
                    </div>
                    <div class="penly-prod">${prodTwh.toFixed(3)} <span class="penly-prod-unite">TWh</span></div>
                    <div class="penly-stats-row">
                        <span>${nbUnites} unités</span>
            ...
            continue; // On empêche la centrale focus d'apparaître dans la grille du bas
        }

        // Construction pour toutes les autres centrales
...
// Fonction pour populer le select la toute première fois
function initialiserSelectMenu(data) {
    const selectMenu = document.getElementById('focus-select');
    // On récupère tous les noms de centrales, triés par ordre alphabétique
    const nomsCentrales = Object.keys(data.production_par_centrale_twh).sort();

    // On évite de rajouter des options si le menu est déjà rempli (ex: au rafraichissement horaire)
    if (selectMenu.options.length === 0) {
        nomsCentrales.forEach(nom => {
        ...
        // On sélectionne par défaut la centrale choisie (PENLY)
        selectMenu.value = centraleEnFocus;

        // On écoute les changements du menu déroulant
        selectMenu.addEventListener('change', function(e) {
            centraleEnFocus = e.target.value;
            rafraichirDashboard(); // On reconstruit l'interface avec le nouveau Focus
        });
    }
}
...
function telechargerDonnees() {
    fetch('data_nucleaire_france.json?t=' + new Date().getTime())
        ...
        .then(data => {
            donneesEnMemoire = data;
            initialiserSelectMenu(data);
            rafraichirDashboard();
        })
        .catch(error => {
...
setInterval(telechargerDonnees, 14400000);
                        </div>
                        <div class="penly-details">
                            ${htmlUnites}
                        </div>
